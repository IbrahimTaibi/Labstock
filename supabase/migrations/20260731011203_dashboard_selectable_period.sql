-- La période du tableau de bord devient un paramètre (en mois) au lieu d'un
-- horizon figé à 6. L'ancienne signature est supprimée : la garder créerait
-- une surcharge ambiguë avec la nouvelle, dont le paramètre a un défaut.

drop function public.dashboard();

create or replace function public.dashboard(p_months integer default 6)
returns jsonb
language sql
stable
security invoker
...
as $$
with params as (
  -- Bornes larges mais finies : au-delà, les séries mensuelles deviennent
  -- illisibles et la reconstruction d'historique coûteuse.
  select least(greatest(coalesce(p_months, 6), 1), 24) as months
),
months as (
  select (date_trunc('month', now())::date - (interval '1 month' * g))::date as start_date
  from params, generate_series((select months from params) - 1, 0, -1) as g
),
bounds as (
  select start_date,
...
select jsonb_build_object(
  ...
  'period', jsonb_build_object(
    ...
    'end', current_date,
    'months', (select months from params)
  )
)
$$;

revoke all on function public.dashboard(integer) from public, anon;
grant execute on function public.dashboard(integer) to authenticated;
