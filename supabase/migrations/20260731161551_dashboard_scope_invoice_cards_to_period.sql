/* « État des factures » et « Top 5 fournisseurs » agrégeaient toutes les
   factures, sans filtre de date : ces deux cartes affichaient la même
   valeur quelle que soit la période choisie. Elles sont désormais bornées
   à la fenêtre observée, comme le reste des séries.

   Restent volontairement sur l'état courant, car ils décrivent une
   situation et non une fenêtre : produits critiques, factures en retard,
   alertes, répartition du stock et indicateurs glissants. */
...
-- Factures émises dans la fenêtre : socle des deux cartes corrigées.
period_invoices as (
  select i.*
  from invoices i, params p
  where i.issue_date between p.start_date and p.end_date
),
...
select jsonb_build_object(
  ...
  'invoiceStatus', (
    select coalesce(jsonb_agg(to_jsonb(t)), '[]'::jsonb) from (
      ...
      from period_invoices group by status
    ) t
  ),
    ...
    select coalesce(jsonb_agg(to_jsonb(t)), '[]'::jsonb) from (
      select s.name as supplier, count(i.id)::int as invoice_count,
             ...
             round(100.0 * sum(i.amount)
                   / nullif((select sum(amount) from period_invoices), 0), 1)::numeric as share
      from suppliers s
      join period_invoices i on i.supplier_id = s.id
      group by s.name
      order by total desc
      limit 5
    ) t
